...
filteringArea columnWidth =
    column
        ...
        , column
            ...
            [ paragraph [ width fill ]
                [ paragraph [ width fill ] [ text <| "The PDA website allows filtering the dataset based on the following, currently implemented criteria: by case-insensitive string-matching a search phrase, by release date, and by maximum similarity to natural proteins. " ] ]
            , paragraph [ width fill ]
                [ paragraph [ width fill ] [ text <| "We have implemented boolean logic in the " ]
                , paragraph [ Font.bold ] [ text <| "text-based search" ]
                , paragraph [ width fill ] [ text <| ": phrases can be separated by AND, OR, and AND NOT (case-sensitive). Search string is split into conditions at the AND separator, and then each condition is evaluated for whether it contains a NOT separator, in which case only results that do not match this condition are returned, or whether it contains an OR separator, in which case only results that match at least one of the conditions separated by OR are returned. To illustrate, intentions behind the example search phrase: " ]
                , paragraph [ width fill, Font.italic ] [ text <| " Woolfson AND coiled-coil OR coiled coil AND NOT 4-helix OR 6-helix" ]
                , paragraph [ width fill ] [ text <| " could be translated as: " ]
                , paragraph [ width fill, Font.italic ] [ text <| " Show me designs that were designed by Woolfson, are coiled(-)coil, and are not 4-helical or 6-helical." ]
                ]
            , paragraph [ width fill ]
                [ paragraph [ Font.bold ] [ text <| "Release date" ]
                , paragraph [ width fill ] [ text <| " can be used to filter as a closed range, or with open upper or lower bound. " ]
                ]
            , paragraph [ width fill ]
                [ paragraph [ width fill ] [ text <| " Filtering " ]
